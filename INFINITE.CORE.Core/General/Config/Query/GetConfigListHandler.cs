...
namespace INFINITE.CORE.Core.Config.Query
{
    ...
    internal class GetConfigListHandler : IRequestHandler<GetConfigListRequest, ListResponse<ConfigResponse>>
    {
        ...
        public async Task<ListResponse<ConfigResponse>> Handle(GetConfigListRequest request, CancellationToken cancellationToken)
        {
            ...
            try
            {
				var query = _context.Entity<INFINITE.CORE.Data.Model.Config>().AsQueryable();

				#region Filter
				Expression<Func<INFINITE.CORE.Data.Model.Config, object>> column_sort = null;
				List<Expression<Func<INFINITE.CORE.Data.Model.Config, bool>>> where = new List<Expression<Func<INFINITE.CORE.Data.Model.Config, bool>>>();
        ...
        #region List Utility
		private (Expression<Func<INFINITE.CORE.Data.Model.Config, bool>> where, Expression<Func<INFINITE.CORE.Data.Model.Config, object>> order) ListExpression(string search, string field, bool is_where)
		{
			Expression<Func<INFINITE.CORE.Data.Model.Config, object>> result_order = null;
			Expression<Func<INFINITE.CORE.Data.Model.Config, bool>> result_where = null;
            if (!string.IsNullOrWhiteSpace(search) && !string.IsNullOrWhiteSpace(field))
            {
                ...
                switch (field)
                {
					...
					case "configkey" : 
						if(is_where){
							result_where = (d=>d.ConfigKey.Trim().ToLower().Contains(search));
						}
						else
							result_order = (d => d.ConfigKey);
					break;
					case "configvalue" : 
						if(is_where){
							result_where = (d=>d.ConfigValue.Trim().ToLower().Contains(search));
						}
						else
							result_order = (d => d.ConfigValue);
					break;
					case "createby" : 
						if(is_where){
